...
<section class="mb-16">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Отзывы о товаре</h2>
        @auth
            <a href="{{ route('reviews.create', ['type' => 'product', 'id' => $product->id]) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md">
    ...
    @if($product->reviews()->approved()->count() > 0)
        <div class="space-y-4">
            @foreach($product->reviews()->approved()->with('user')->latest()->get() as $review)
    ...
    @else
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md text-center">
            <p class="text-gray-600 dark:text-gray-300">Пока нет отзывов о данном товаре.</p>
            @auth
